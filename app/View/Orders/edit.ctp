...
// ***************************************************************************
	...
	$this->set('title_for_layout', __('Édition de la commande n°%s', $this->request->data['Order']['id']));

echo $this->Form->create('Order', array('inputDefaults' => array('div' => false)));

echo 'Modification de la commande ' . $this->request->data['Order']['id'] . ' | '; 
echo $this->Form->input('library_id', array('options' => $libraries, 'label' => __('Attribué à la bibliothèque '))) . ' ';
...
<div class="box">
	<div class="box-content">
		<?php
			echo $this->Form->input('status_id', array('options' => $status, 'label' => __('Statut') . ' :'));
			echo $this->Form->input('origin_id', array('options' => $origins, 'label' => __('Provenance') . ' :'));
			echo $this->Form->input('priority', array('options' => array('2' => __('Normale'), '1' => __('Urgente'), '3' => __('Pas prioritaire')), 'label' => __('Priorité') . ' :'));
...
<div class="box">
	<div class="box-content">
		<?php
			...
			echo $this->Form->input('deliver_type', array('type' => 'radio', 'options' => array('mail' => __('envoi par email (facturé)'), 'surplace' => __('m\'avertir et je passe faire la copie (non facturé)')), 'legend' => false));
			echo '<br />';
		?>
	</div>
</div>
...
<div class="box">
	<div class="box-content">
			...
			<div class="box-content">
				<center>
					<?php echo __('Remplir la commande à partir du '); ?>
						...
						<button type="submit" id="resolveSubmit" onclick="return false;" class="btn">OK</button>
					<?php 
		...
		<div id="resultForm">
			...
			<?php
			echo $this->Form->input('user_comment', array('label' => __('Remarques') . ' :'));
			?>
		</div>
		<?php
		echo $this->Form->button(__('Enregistrer'), array('class' => 'btn'));
		echo $this->Form->end();
		if(AuthComponent::user('admin_level') == '1' || AuthComponent::user('admin_level') == '2')
		{
			echo $this->Form->postlink(__('Supprimer'), array('controller' => 'orders', 'action' => 'delete', $this->request->data['Order']['id']), array('confirm' => __('Etes-vous sûr de vouloir supprimer la commande %s?', h($this->request->data['Order']['id'])), 'class' => 'btn'));
		}
		?>
	</div>
</div>
