...
// ***************************************************************************
	...
	$this->set('title_for_layout', __('Édition du service "%s"', h($this->request->data['Service']['name'])));

	echo '<div class="manage_title">', __("Gestion des services : Édition de la fiche"), ' ', $this->request->data['Service']['id'], '</div>';
	...
	echo '<div class="manage_footer">';
		...
		echo $this->Html->link(__('Annuler'), array('controller' => 'services', 'action' => 'index'), array('class' => 'btn'));
		echo $this->Form->end();
		echo $this->Form->postlink(__('Supprimer'), array('action' => 'delete', $this->request->data['Service']['id']), array('confirm' => __('Etes-vous sûr de vouloir supprimer le service "%s"?', h($this->request->data['Service']['name'])), 'class' => 'btn'));
	echo '</div>';
?>